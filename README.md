...
本库在 `zpk2sos()` 中对**每个二阶节单独**做参考频率处归一化（`norm_gain()`），使每个节的参考频率幅值为 1。这与 scipy 的做法不同——scipy 计算原型增益 `k = prod(-p)`，随频域变换和双线性变换传播，最终将累积的 `k` 一次性乘入第一节分子。

**影响**：
- **Butterworth**：不受影响。原型 DC 增益恒为 1。
- **Chebyshev I 偶数阶**：原型 DC 增益为 `1/√(1+ε²)`（-rp dB），逐节归一化强制拉到 0 dB，导致通带纹波整体上移，通带峰值超过 0 dB，不满足纹波在 `[0, -rp]` dB 之间的标准定义。
- **Chebyshev II**：不受影响。原型 DC 增益恒为 1。
- 所有滤波器的**频率响应形状**（纹波幅度、过渡带宽度）不受影响，仅整体幅值有偏移。

### float32 无法支撑完整的 zpk 增益传播

尝试按 scipy 方式全程追踪增益 `k` 时发现，`bilinear_transform` 需计算 `∏(2fs - s_i)` 作为增益修正因子。对于高阶带阻/带通滤波器（如 7 阶 BS），频率变换后极点数量多（14 个），s 域极点幅值大，连乘积超过 `float32` 最大值（~3.4×10³⁸），导致 `inf`，后续 `inf/inf = NaN`。

这意味着在 `float32` 精度下**无法实现完整的原型增益传播**。scipy 使用 `float64` 无此问题。若未来要修复偶阶 Chebyshev I 的纹波偏移问题，需将增益追踪路径上的关键变量改为 `double`。
